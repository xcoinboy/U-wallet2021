...
// Copyright (c) 2016 The UrbanCash developers
...
const QString BITCOIN_IPC_PREFIX("urbancash:");

static QString ipcServerName()
{
    QString name("UrbanCash");
...
PaymentServer::PaymentServer(QApplication* parent) : QObject(parent), saveURIs(true)
{
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start urbancash: click-to-pay handler");
    else
...
{
    // clicking on urbancash: URLs creates FileOpen events on the Mac:
